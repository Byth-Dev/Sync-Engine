...
class PlayState extends MusicBeatState
{
	...
	private var sickWindow:Float = 45;
	private var goodWindow:Float = 90;
	private var badWindow:Float = 135;
	...
	override public function create()
	{
		...
		engineWatermark = new FlxText(4, healthBarBG.y + 50, 0, "Sync Engine v1.0", 25);
		engineWatermark.setFormat(Paths.font("vcr.ttf"), 25, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		...
		scoreTxt = new FlxText(FlxG.width / 2 - 235, healthBarBG.y + 50, 0, "", 25);
	...
	override public function update(elapsed:Float)
	{
		...
		{
			scoreTxt.text = "Score: " + songScore + " | Misses: " + misses + " | Accuracy: " + formatFloat(accuracy, 2) + "%";
		}
	...
	private function keyShit():Void
	{
		...
		if (pressArray.contains(true) && /*!boyfriend.stunned && */ generatedMusic)
		{
			...
			// When ghost tapping is disabled
		    if (PreferencesMenu.getPref('ghost-tap'))
			{
				for (shit in 0...pressArray.length)
			    { // Ghost tapping disabled
	...
	function formatFloat(value:Float, decimals:Int):String // function used for caculating accuracy i guess
	{
	...
	function addAccuracy(rating:String):Void
	{
		totalNotes += 1.0;
		...
		if (totalNotes > 0.0)
		{
			accuracy = (totalNotesHit / totalNotes) * 100.0;
		}
	}
	...
	function noteAltMiss(direction:Int = 1):Void
	{
		// this is used for note miss
		misses += 1;
		songScore -= 10;
		killCombo();
		addAccuracy("miss");
	...
	function noteCancelMiss(direction:Int = 1):Void
	{
		// A dupe function for ghost tapping
		health += 0;
	}
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if (noteDiff <= sickWindow)
			{
				addAccuracy("sick");
			}
			else if (noteDiff <= goodWindow)
			{
				addAccuracy("good");
			}
			else
